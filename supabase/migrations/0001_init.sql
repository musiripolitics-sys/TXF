-- ============================================================
-- Techxfluence — initial schema
-- Postgres / Supabase. Run in Supabase SQL Editor or via CLI.
-- ============================================================
...
-- is_admin() — used by RLS write policies. SECURITY DEFINER to read user_roles.
...
-- ============================================================
-- Payments (created before events/registrations/sponsorships)
-- ============================================================
create table public.payments (
  ...
  provider_ref text,
  related_type text,                          -- 'registration'|'membership'|'sponsorship'|'listing_fee'
  related_id   uuid,
  created_at   timestamptz not null default now()
);
...
create table public.events (
  ...
  price_amount  int not null default 0,       -- minor units
  currency      char(3) not null default 'INR',
  blurb         text,
  about         text,
  ...
  submission_id uuid,                          -- FK added after host_submissions
...
-- close the events <-> host_submissions circular reference
...
create table public.membership_plans (
  ...
  currency     char(3) not null default 'INR',
  cadence      text,                           -- 'forever' | 'per month'
  tagline      text,
...
create table public.plan_benefits (
  ...
  perk_text  text,                             -- free-text perk if not a benefit row
...
create policy "anyone submit host proposal" on public.host_submissions
  for insert with check (true);
create policy "anyone submit contact" on public.contact_messages
  for insert with check (true);
create policy "anyone subscribe newsletter" on public.newsletter_subscribers
  for insert with check (true);
create policy "anyone register" on public.registrations
  for insert with check (true);
...
-- Users see their own registrations / memberships / badges / points
...
-- Admins: full management on the operational tables
